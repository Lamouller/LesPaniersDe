      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">{MESSAGES.filter((m) => m.unread).length} non lu(s)</p>
          </div>
        </div>
              ...
              <CardContent className="pt-5 pb-5">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-start gap-3">
                    <div className="w-9 h-9 rounded-full bg-white/10 flex items-center justify-center text-sm font-bold text-foreground/70 flex-shrink-0">
                      {msg.client[0]}
                    </div>
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        <p className="text-sm font-semibold text-foreground/80">{msg.client}</p>
                        {msg.unread && (
                          <span className="w-2 h-2 rounded-full bg-white" />
                        )}
                      </div>
                      <p className="text-xs text-muted-foreground/60 mb-1">{msg.entity}</p>
                      <p className="text-sm text-muted-foreground truncate">{msg.preview}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-1.5 flex-shrink-0 text-xs text-muted-foreground/60">
        ...
        <div className="mt-8 text-center">
          <p className="text-xs text-muted-foreground/60">Phase 2 — Interface de réponse complète à venir</p>
        </div>
      </div>
    </>
  );
